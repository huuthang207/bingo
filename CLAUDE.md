...
- Backend CORS defaults to `WEB_ORIGIN=http://localhost:3000` when `WEB_ORIGIN` is unset.
...
- `apps/server`: Express REST API, Socket.IO handlers, Prisma access, token utilities, room code generation, and server-owned Bingo logic.
...
```text
Next.js frontend
  -> REST API for creating rooms, joining rooms, and state recovery
...
- `apps/server/src/app.ts` creates the Express app, JSON parsing, CORS, `/health`, and `/rooms` routes.
...
- `apps/web/src/components/BingoBoard.tsx` and `CalledItemCard.tsx` are shared UI components for board display and called item rendering.
...
- `Player`: player token hash, player board JSON, marked cells JSON, winner state, timestamps.
