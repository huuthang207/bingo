...
export function PlayRoomClient({ roomCode }: PlayRoomClientProps) {
  ...
  return (
    <PageShell className="py-4 pb-24 sm:py-5 sm:pb-6 lg:pb-5">
      <div className="mx-auto grid max-w-7xl gap-4 lg:grid-cols-[minmax(18rem,0.72fr)_minmax(0,1.28fr)] lg:items-start">
        <aside className="space-y-3 sm:space-y-4 lg:sticky lg:top-5">
          ...
          <section className="grid gap-2 sm:gap-3">
            <CalledItemCard animate={latestItem?.calledOrder === animatedCalledOrder} compact item={latestItem?.item} key={latestItem?.calledOrder ?? "empty"} previousItem={previousItem?.item} />
          </section>
        ...
        <section className="grid gap-3 lg:justify-items-center">
          {board.length > 0 ? (
            <div className="w-full lg:max-w-[min(100%,calc(100vh-8.5rem))]">
              <BingoBoard board={board} calledItemIds={calledItemIds} markedCells={markedCells} onCellClick={markCell} disabled={roomStatus !== "playing" || !isConnected} />
            </div>
          ) : null}

          <div className="hidden w-full lg:max-w-[min(100%,calc(100vh-8.5rem))] sm:block">
